...
namespace Books.WebUI.Infrastructure
{
    public class Text
    {
        ...
        public void StartNewParagraph(ref int ParagraphsNoOf, ref List<string> Paragraphs, string sentence, ref string paragraph, char currChar, ref int ParagraphLength, bool Debug, string DebugText, bool Format, bool ItsAnOrderedList, bool ItsAnUnorderedList, bool ItsGotHTML, bool SplittingOnColon, bool InsertIndicators)
        {
            if (Debug)
            {
            ...
            // Emit existing list heading and this will be formated using HTML (i.e. include the "¥" and skip the "1." etc.)
            ...
            //							
            //Format    Indicators	SpltOnColon OL	UL	HTML    Other   Action                  Comment
            //	
            // Y    	Y	        N       	Y	N	N   	N   	Append "¤" & currChar   (Format Ordered List)
            //			                        N   Y	N   	N   	Append "¥" & currChar   (Format Unordered List)
            //			                        N   N	Y   	N   	Append currChar         (Process any HTML)
            //			                        N   N	N   	Y   	Append currChar         (Normal - append char to paragraph)
            //		                Y	        Y   N	N   	N   	Append "§" alone        (Treat as code, skip ":")
            //			                        N   Y	N   	N   	Append "§" alone        (Treat as code, skip ":")
            //			                        N   N	Y   	N   	Append "§" alone        (Treat HTML as code, skip ":")
            //			                        N   N	N   	Y   	Append "§" alone        (Treat as code, skip ":")
            //          N   	    N	        Y   N	N   	N   	Append currChar         (Normal - append char to paragraph)
            //			                        N   Y	N   	N   	Append currChar         (Normal - append char to paragraph)
            //			                        N   N	Y   	N   	Append currChar         (Process any HTML)
            //			                        N   N	N   	Y   	Append currChar         (Normal - append char to paragraph)
            //		                Y	        Y   N	N   	N   	Append "" alone         (Dont treat as code, skip ":")
            //			                        N   Y	N   	N   	Append "" alone         (Dont treat as code, skip ":")
            //			                        N   N	Y   	N   	Append "" alone         (Process any HTML, skip ":")
            //			                        N   N	N   	Y   	Append "" alone         (Dont treat as code, skip ":")
            //N	        Y	        N       	Y	N	N   	N   	Append "§" & currChar   (Treat paragraph as code)
            //			                        N   Y	N   	N   	Append "§" & currChar   (Treat paragraph as code)
            //			                        N   N	Y   	N   	Append currChar         (Process HTML even in unfmt'd txt)
            ...
            if ((Format && InsertIndicators && SplittingOnColon) ||
            ...
            if ((Format && InsertIndicators && !SplittingOnColon && !ItsAnOrderedList && !ItsAnUnorderedList) ||
                (Format && !InsertIndicators && !SplittingOnColon) ||
        ...
        public bool Formatting(ref bool format, string firstWord, char firstWordChar, string secondWord, char lastCharInLine, bool ItsAList, ref bool SplittingOnAColon, string newLine)
        {
            if (format)
            {
                ...
                // "@{",
                if (firstWord == ">" || firstWord == "{" || firstWord == "$" || firstWord == "..." || firstWord == "//" || firstWord == "namespace" || firstWord == "@using" || firstWord == "@model" || firstWord == "@{" || firstWord == "git" || firstWord == "USE" || firstWord == "SELECT" || firstWord == "FROM" || firstWord == "ORDER" || firstWord == "GROUP" || firstWord == "INTO" || firstWord == "WHERE" || firstWord == "INSERT" || firstWord == "VALUES" || firstWord == "DELETE" || firstWord == "CREATE" || firstWord == "DROP" || firstWord == "INNER" || firstWord == "OUTER" || firstWord == "JOIN" || firstWord == "UPDATE" || firstWord == "SET" || firstWord == "WHEN" || firstWord == "MERGE" || firstWord == "OFFSET")
                {
                    format = false;
                }
                // < for HTML
                // [ for C# properties e.g. <=== turned this off
                ...
                if (firstWord == "public" || firstWord == "private" || firstWord == "protected" || firstWord == "using" || firstWord == "CREATE")
                {
                    if (secondWord == "class" || secondWord == "void" || secondWord == "int" || secondWord == "bool" || secondWord == "string" || secondWord == "override" || secondWord == "interface" || secondWord == "TABLE")
                    {
                        format = false;
                    }
            ...
            else
            {
                ...
                {
                    if (lastCharInLine != ';' && lastCharInLine != '{' && lastCharInLine != '>' && newLine.Substring(0, 1).ToCharArray()[0] == firstWordChar && firstWord != "USE" && firstWord != "SELECT" && firstWord != "FROM" && firstWord != "ORDER" && firstWord != "GROUP" && firstWord != "INTO" && firstWord != "WHERE" && firstWord != "INSERT" && firstWord != "VALUES" && firstWord != "DELETE" && firstWord != "CREATE" && firstWord != "DROP" && firstWord != "INNER" && firstWord != "OUTER" && firstWord != "JOIN" && firstWord != "UPDATE" && firstWord != "SET" && firstWord != "WHEN" && firstWord != "MERGE" && firstWord != "OFFSET")
                    {
                        format = true;
                    }
                }
                // Quite often a single line ending in : is not code or HTML but is treated as code if it doesnt start with a
                // capital letter
                //if (lastCharInLine == ':')
                //{
                //    formatAgain = true;
                     ...
                     ))
                {
                    ...
                    if (lastChar == (Char)10)
                    {
                        ...
                        if (newLine.Count<Char>(x => x == '<') > 1 && newLine.Contains(">") && newLine.Contains("</"))
                            { ItsGotHTML = true; } else { ItsGotHTML = false; }
                        ...
                        FormatBefore = Format;
                        Format = Formatting(ref Format, firstWord, firstWord1stChr, secondWord, lastCharInLine, ItsAList, ref SplittingOnColon, newLine);
                    ...
                    if (currChar == ':' && nextChar == (Char)13 && !FirstLine && Format && SplitOnColon)
                    {
                    ...
                    {
                        endOfParagraph = false;
                        StartNewParagraph(ref ParagraphsNoOf, ref Paragraphs, sentence, ref paragraph, currChar, ref ParagraphLength, Debug, DebugText, Format, ItsAnOrderedList, ItsAnUnorderedList, ItsGotHTML, SplittingOnColon, InsertIndicators);
                    }
                    else
                    {
                        if (!endOfParagraph)
